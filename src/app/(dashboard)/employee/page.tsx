      ...
      {open && (
        <div className="absolute top-full mt-2 left-0 z-50 min-w-40 rounded-2xl border border-slate-200 dark:border-white/10 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl shadow-xl shadow-slate-900/10 dark:shadow-black/40 overflow-hidden">
          {options.map((opt) => (
            ...
            >
              {opt.value === value && (
                <svg className="w-3 h-3 text-sky-500 shrink-0" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
            ...
            <div className="flex items-center gap-2 self-start sm:self-auto">
              ...
              <div className="relative">
                <svg className="w-4 h-4 text-slate-500 dark:text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              [0, 1, 2, 3, 4].map((item) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {[0, 1, 2, 3, 4].map((day) => (
                      <div key={day} className="h-6 w-8 rounded-lg bg-slate-200 dark:bg-white/10" />
                    ))}
                  </div>
                </div>
              ))}
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {WEEKDAYS.map((day) => (
                      <span
                        key={day}
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-lg border ${shift.days && shift.days.includes(day)
                    ...
                    >
                      {STATUS_CONFIG[shift.status as ShiftStatus]?.label || shift.status}
                    </span>
                  </div>
                </div>
              ))}
